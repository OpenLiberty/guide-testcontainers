...
You'll develop integration tests capable of running in both dev mode and a CI/CD pipeline. These flexible tests ensure the application's reliability across diverse environments.

Navigate to the `start` directory to begin.

Running the PostgreSQL database is integral for the `inventory` microservice, as it handles the persistence of data.
...
Now that the setup is complete, you can start writing the tests that use the `SystemResourceClient` REST client and the Testcontainers integration.

In this section, you'll learn how to create test cases using the test REST client and Testcontainers. You'll set up a multi-container test environment, determine the appropriate runtime environment to launch Testcontainers, and configure the test REST client for HTTPS communication.
...
Next, you'll learn to prepare your Maven project for test execution. You'll add the required dependencies for Testcontainers and logging, set up Maven to copy the PostgreSQL JDBC driver during the build phase, and configure the Liberty Maven Plugin to handle PostgreSQL dependencies.
